...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--results", type=Path, default=BASE_DIR / "results_people")
    ap.add_argument("--out", type=Path, default=BASE_DIR / "eval_out_people")
...
def run(args: argparse.Namespace) -> None:
    ...
    if not df.empty:
        for model_name, g in df.groupby("model"):
            ...
            if args.include_teacher_runs:
                ...
                weighted_events = weighted_mean(g2["specific_events_macro_acc"], g2["teacher_consensus_weight"])
                weighted_risk = weighted_mean(g2["risk_assessment_macro_acc"], g2["teacher_consensus_weight"])
            ...
            agg_rows.append(
                {
                    ...
                    "weight_specific_events_macro_acc_mean": weighted_events,
                    "weight_risk_assessment_macro_acc_mean": weighted_risk,
                }
            )
    ...
    if not df_agg.empty:
        df_agg = df_agg.sort_values(
            by=["rule_rate", "global_slot_macro_acc_mean", "risk_assessment_macro_acc_mean", "specific_events_macro_acc_mean"],
            ascending=False,
        )
