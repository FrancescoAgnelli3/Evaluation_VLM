...
"""Local manifest-based reasoner SFT recipe on Qwen3-VL.

Consumes a dataset root in the LocalSFTDataset layout:
...
The text JSON files must contain the canonical ``{"conversations": [...]}``
structure expected by ``LocalSFTDataset``.
"""
...
class _UnshardedLocalSFTDataset(LocalSFTDataset):
    """Avoid double-sharding when wrapped by DataPackerDataLoader."""
...
def build_local_reasoner_dataset(split: str = "train") -> _UnshardedLocalSFTDataset:
    dataset_root = os.environ["DATASET_PATH"]
    manifest_path = os.path.join(dataset_root, "meta.json")
    return _UnshardedLocalSFTDataset(
        ...
        is_infinite_loader=(split == "train"),
        split=split,
        dataset_name="local_reasoner_sft",
    )


def build_local_reasoner_datapacker_dataloader(**kwargs) -> DataPackerDataLoader:
...
class LocalReasonerDataPacker(DataPacker):
    ...
    def sft_collate_fn(
        ...
        assert len(samples) == 1, f"LocalReasonerDataPacker expects max_batch_size=1, got {len(samples)}"
        s = samples[0]
...
local_reasoner_sft = LazyDict(
    dict(
        ...
        dataloader_train=L(build_local_reasoner_datapacker_dataloader)(
            data_source=L(build_local_reasoner_dataset)(
                split="train",
            ),
            data_packer=L(LocalReasonerDataPacker)(
...
for _item in [local_reasoner_sft]:
